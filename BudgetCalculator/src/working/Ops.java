...
public class Ops {
	...
	private static void ParseTxt() throws NullPointerException, NumberFormatException, StringIndexOutOfBoundsException {
		...
		try (BufferedReader br = Files.newBufferedReader(Paths.get(Gui.filePath))) {
			...
			Logger.error(
					"--- IllegalArgumentException occurred in the ParseTxt method, check the AddToAppropriateArrayList and AddTransactionArrayListsToHashMap methods");
			iae.printStackTrace();
			System.exit(1);
		} catch (Exception ex) {
	...
	// Establishes monthly income then breaks down input into a 50/40/5/5 split for
	// budgeting reasons.
	...
	private static String TotalMonthlySpendingString(BigDecimal monthlyExpenseTotal, BigDecimal monthlySavingsTotal,
			BigDecimal monthlyInvestmentTotal, BigDecimal monthlyEmergencyReserveTotal) {
		return MessageFormat.format(
				"This month, you spent in total, Expense: ${0}, Saving: ${1}, Investment: ${2}, Emergency: ${3}" + "\n",
				monthlyExpenseTotal, monthlySavingsTotal, monthlyInvestmentTotal, monthlyEmergencyReserveTotal);
	}

	private static String RemainingMoneyString(BigDecimal remainingExpenseAmount, BigDecimal remainingSavingsAmount,
			BigDecimal remainingInvestmentAmount, BigDecimal remainingEmergencyReserveAmount) {
		return MessageFormat.format(
				"After calculating the monthly expenses, savings, investments and emergencies based on a 50/40/5/5 breakdown, "
						+ "\n" + "Expense: ${0}" + "\n" + "Savings: ${1}" + "\n" + "Invest: ${2}" + "\n"
						+ "Emergency Reserve: ${3}" + "\n",
				remainingExpenseAmount, remainingSavingsAmount, remainingInvestmentAmount,
				remainingEmergencyReserveAmount);
	}
